...
Endpoints:
  ...
  GET  /api/manufacturing/materials        — full materials catalog (80+ entries)
  GET  /api/manufacturing/materials/{name} — look up material by name or alias
"""
...
@router.get(
    "/materials",
    summary="List all materials in the local engineering catalog",
    response_model=Dict[str, Any],
)
async def list_materials(family: Optional[str] = None) -> Dict[str, Any]:
    """
    Returns the full materials catalog (80+ entries).
    Optionally filter by material family: ferrous, non_ferrous, polymer,
    composite, ceramic, refractory, etc.
    """
    from manufacturing.materials_catalog import (
        MATERIALS_CATALOG,
        materials_by_family,
    )

    if family:
        entries = materials_by_family(family.lower())
        return {
    ...
    all_entries = list(MATERIALS_CATALOG.values())
    families = sorted({e["family"] for e in all_entries})
    return {
...
@router.get(
    "/materials/{name}",
    summary="Look up a specific material by name or common alias",
    response_model=Dict[str, Any],
)
async def get_material(name: str) -> Dict[str, Any]:
    """
    Returns engineering data for a specific material.
    Accepts canonical names or common aliases (e.g. "Ti-6Al-4V", "mild steel", "6061").
    Falls back to web + LLM research if the material is not in the local catalog.
    """
    from manufacturing.materials_catalog import get_material as _get_material, search_materials
    from manufacturing.agent import research_material

    entry = _get_material(name)
    if entry:
        return {"source": "local_catalog", "material": entry}

    # Try fuzzy search within catalog
    hits = search_materials(name)
    if hits:
        return {"source": "local_catalog_fuzzy", "query": name, "material": hits[0], "other_matches": hits[1:3]}

    # Fall back to web + LLM research
    researched = research_material(name)
    if researched:
        return {"source": "web_research", "material": researched}

    raise HTTPException(
        status_code=404,
        detail=f"Material '{name}' not found in catalog or web research.",
    )
